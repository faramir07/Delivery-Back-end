...
export const checkLoginClient = async (
  ...
  if (!validateUserClient) {
    res.status(404).send({ error: "email y contraceña incorrecta" });
    return;
  }
  ...
  if (validateUserClient.state !== "active") {
    res.status(403).send({
      error: "Tu cuenta está inactiva o bloqueada, revisa tu email para activar la cuanta o comunícate con soporte técnico",
    });
    return;
  }
...
export const checkLoginDelivery = async (
  ...
  if (base === undefined || base < 50000) {
    res
      .status(401)
      .send({ error: "deves poseer una base de por lo menos de 50 mil" });
    return;
  }
